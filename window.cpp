...
namespace szuKacz
{
			...
			case WM_NOTIFY:
			{
				switch(((LPNMHDR)lParam)->code)
				{
					case NM_DBLCLK:
					{
						...
						if(pos != -1)
						{
							if(GetDefaultAction(szuKacz::results[pos].cnt) != -1)
							{
								CallAction(IMIG_CNT, GetDefaultAction(szuKacz::results[pos].cnt), szuKacz::results[pos].cnt);
							}
						}
						break;
					}
				...
				else if(lParam == (LPARAM)szuKacz::toolbar)
				{
					switch(LOWORD(wParam))
					{
						case 1:
						{
							...
							if(pos != -1)
							{
								if(GetDefaultAction(szuKacz::results[pos].cnt) == IMIA_CNT_MSG)
								{
									CallAction(IMIG_CNT, IMIA_CNT_MSG, szuKacz::results[pos].cnt);
								}
							}
							break;
						}
						case 2:
						{
							...
							if(pos != -1)
							{
								CallAction(IMIA_NFO_DETAILS_NET, IMIA_MSG_INFO, szuKacz::results[pos].cnt);
							}
							break;
						}
					}
	...
	void ListView(HWND parent)
	{
		//tworzymy kontrolkê
		{
			szuKacz::listview = CreateWindowEx(WS_EX_CLIENTEDGE, WC_LISTVIEW, "", WS_CHILD|WS_VISIBLE|WS_HSCROLL|LVS_SHOWSELALWAYS|LVS_REPORT|LVS_SHAREIMAGELISTS, 0, 0, 0, 0, parent, 0, Ctrl->hInst(), 0);
	...
	void Toolbar(HWND parent)
	{
		...
		//dodajemy przyciski
		{
			...
			SendMessage(szuKacz::toolbar, TB_ENABLEBUTTON, 1, 1);
			SendMessage(szuKacz::toolbar, TB_ENABLEBUTTON, 2, 1);
		}
	}
}